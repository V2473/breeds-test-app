...
export default function BreedPage({
  ...
  const API_KEY =
    id.length > 3 ? process.env.CAT_API_KEY : process.env.DOG_API_KEY;
  const infoUrl =
    id.length > 3
  ...
  useEffect(() => {
    const fetchBreedDetails = async () => {
      try {
        ...
        if (!responseBreedDetails.ok) {
          throw new Error(
            `Failed to fetch breed details, ${responseBreedDetails.status}`,
          );
        }
  ...
  useEffect(() => {
    const fetchBreedImageUrls = async () => {
      try {
        ...
        if (!imgLinksResponse.ok) {
          throw new Error(
            `Failed to fetch breed image, ${imgLinksResponse.status}`,
          );
        }
  ...
  const descriptionStart = breedInfo.indexOf(
    '"',
    breedInfo.indexOf(`${breedDataText}`) + 12,
  );
  const descriptionEnd = breedInfo.substring(
    descriptionStart + 1,
    breedInfo.indexOf('"', descriptionStart + 1),
  );
  ...
  imgDataParsed.split(',').forEach((imgUrl) => {
    if (imgUrl.length > 8) {
      allPhotos.push(imgUrlDirect + imgUrl + '.jpg');
    }
  });
      ...
      {allPhotos.length > 0 ? (
        allPhotos.map((img, index) => (
          <img
            ...
            className="max-h-80 object-cover w-80 m-2"
          />
        ))
      ) : (
